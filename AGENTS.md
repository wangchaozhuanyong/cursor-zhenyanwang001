...
- 固定一个本地测试入口：`http://127.0.0.1:5174`。同一时间只用这个入口验收一个当前候选分支，避免多个窗口看到不同页面。
- 多窗口开发时，每个窗口可以在自己的 worktree/分支里改代码，但不要各自启动新的“正式测试入口”。需要用户验收时，先把候选分支切到统一测试目录或合并到本地 `main` 候选区，再启动/刷新 `5174`。
