...
PWA 安装入口集中在客服下载页面：`/support-download?tab=download`。`/install` 会跳转到该页面。首页、个人中心和全站布局不提供安装入口。
...
- 首页配置、站点信息、客服下载配置：`StaleWhileRevalidate`
...
4. 打开首页、分类页、商品列表、商品详情、客服下载页两次，第二次应优先显示缓存内容并后台刷新。
