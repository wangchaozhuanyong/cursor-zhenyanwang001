...
## 更新与离线
新版本只提示用户手动刷新，不强制刷新。用户点「稍后」后，当前 `sw.js` 版本会记入 `sessionStorage`，同一部署版本不会重复弹窗；真正有新部署（`index.html` precache revision 变化）时会再次提示。SW 注册为延后执行（`immediate: false`），并仅在切回标签页或约每小时检查一次更新，避免一进站就频繁打扰。

离线时公开页面可显示缓存或 `/offline.html`，订单、支付、购物车、个人资料和后台管理需要联网。
