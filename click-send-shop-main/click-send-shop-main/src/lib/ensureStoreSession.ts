 ...
 */
export async function ensureStoreSession(options?: { allowCookieProbe?: boolean }): Promise<boolean> {
  const flagged = isLoggedIn();
  const storeAuthenticated = useAuthStore.getState().isAuthenticated;
  if (!flagged && !storeAuthenticated && !options?.allowCookieProbe) return false;

  const { restoreSessionFromCookie } = await import("@/services/authService");
  const ok = await restoreSessionFromCookie({ allowWithoutLoginHint: options?.allowCookieProbe });
  if (!ok) {
