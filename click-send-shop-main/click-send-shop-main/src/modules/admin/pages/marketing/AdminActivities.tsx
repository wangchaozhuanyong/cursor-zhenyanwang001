  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="flex items-center gap-2 text-xl font-bold"><CalendarClock className="h-5 w-5 text-[var(--theme-price)]" />活动管理 / 营销活动</h1>
          <p className="text-xs text-muted-foreground">活动列表与运营动作入口。</p>
        </div>
        <PermissionGate permission="activity.manage">
          <button type="button" onClick={() => navigate("/admin/marketing/activities/new")} className="rounded-lg bg-gold px-4 py-2.5 text-sm font-semibold text-primary-foreground"><PlusCircle className="mr-1 inline h-4 w-4" />新建活动</button>
        </PermissionGate>
      </div>
      ...
      <div className="grid gap-3 md:grid-cols-[1fr_160px_auto]">
        ...
        <select value={type} onChange={(e) => setType(e.target.value as ActivityType | "")} className="rounded-lg bg-secondary px-3 py-2 text-sm"><option value="">全部类型</option><option value="flash_sale">限时秒杀</option><option value="full_reduction">满减活动</option></select>
