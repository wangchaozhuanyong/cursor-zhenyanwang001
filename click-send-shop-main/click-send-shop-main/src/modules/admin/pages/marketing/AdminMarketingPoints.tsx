...
export default function AdminMarketingPoints() {
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-xl font-bold text-foreground">活动管理 / 积分管理</h1>
      ...
      {tab === "积分规则" ? <div className="rounded-xl border border-border bg-card p-4 text-sm text-muted-foreground">积分规则维护请在本页规则区编辑；此处预留独立规则配置面板。</div> : null}
      {tab === "积分明细" ? <AdminPointsRecords /> : null}
      {tab === "手动调整" ? <div className="rounded-xl border border-border bg-card p-4 text-sm text-muted-foreground">手动调整入口已预留，后续将接入用户选择、增减值、原因与审计链路。</div> : null}
    </div>
  );
}
