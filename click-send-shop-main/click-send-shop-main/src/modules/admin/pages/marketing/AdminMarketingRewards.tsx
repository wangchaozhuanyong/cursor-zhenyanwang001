...
export default function AdminMarketingRewards() {
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-xl font-bold text-foreground">活动管理 / 返现管理</h1>
      ...
      {tab === "返现规则" ? <div className="rounded-xl border border-border bg-card p-4 text-sm text-muted-foreground">返现规则配置区预留（邀请返现/订单返现/层级返现）。</div> : null}
      {tab === "返现记录" ? <AdminRewardRecords /> : null}
      {tab === "提现/结算" ? <div className="rounded-xl border border-border bg-card p-4 text-sm text-muted-foreground">提现/结算能力预留。</div> : null}
    </div>
  );
}
