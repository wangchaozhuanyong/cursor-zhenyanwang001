        ...
        <select className="rounded border px-3 py-2 text-sm" value={severity} onChange={(e) => { setPage(1); setSeverity(e.target.value); }}>
          {severities.map((s) => (
            <option key={s} value={s}>
              {s ? formatMonitoringSeverityLabel(s) || s : "全部等级"}
            </option>
          ))}
        </select>
      ...
      <div className="overflow-x-auto rounded border border-slate-200 bg-white">
        <table className="w-full min-w-[1100px] text-left text-sm">
          <thead className="bg-slate-50 text-slate-500">
            <tr><th className="p-3 w-16">等级</th><th className="p-3 w-20">模块</th><th className="p-3">异常标题</th><th className="p-3">关联对象</th><th className="p-3">可能原因</th><th className="p-3 whitespace-nowrap">首次发现</th><th className="p-3 whitespace-nowrap">最近发现</th><th className="p-3 w-24">状态</th><th className="p-3 w-[11rem]">操作</th></tr>
          </thead>
          <tbody>
            {list.map((item) => (
