...
export const REPORT_REGISTRY: ReportRegistryItem[] = [
  ...
  },
  {
    ...
    kpiProfile: "sales",
    kpiPriorityKeys: REPORT_KPI_PRIORITIES.sales,
    warningKeys: ["missing_cost_order_count"],
    columns: ["date", "order_count", "paid_order_count", "gross_sales", "net_sales", "refund_amount", "discount_amount", "average_order_value", "payment_rate", "refund_rate"],
    shellType: "generic",
    exportable: true,
    dataScopeNote: "销售日报仅将已支付订单计入销售额，退款和优惠按订单财务字段汇总。",
    maxKpis: 10,
  },
  {
    key: "sales_monthly",
  ...
  },
  {
    ...
    shellType: "generic",
    exportable: true,
    dataScopeNote: "客户分析当前以订单用户和付款用户为统计口径；没有行为表时不使用“活跃用户”误导。",
    maxKpis: 8,
  },
  {
  ...
  },
  {
    ...
    filters: ["dateRange", "couponId"],
    kpiProfile: "coupon",
    kpiPriorityKeys: REPORT_KPI_PRIORITIES.coupon,
    warningKeys: [],
    columns: ["coupon_title", "issued_count", "claimed_count", "used_count", "expired_count", "claim_rate", "use_rate", "paid_order_count", "sales_amount", "discount_amount", "net_sales", "gross_profit_amount", "roi"],
    shellType: "generic",
    exportable: true,
