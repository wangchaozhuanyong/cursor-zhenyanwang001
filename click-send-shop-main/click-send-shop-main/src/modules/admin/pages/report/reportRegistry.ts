...
import {
  ...
  fetchDiscountCostReport,
  fetchInventoryAnalysisReport,
  fetchInventoryOccupancyReport,
  fetchOrderAnalysisReport,
  fetchOrderCancelReasonReport,
  fetchPaymentFailureReport,
  fetchProductAnalysisReport,
  fetchPromotionConversionReport,
...
export const REPORT_REGISTRY: ReportRegistryItem[] = [
  ...
  {
    key: "inventory_occupancy",
    reportKey: "inventory_occupancy",
    title: "库存占用",
    description: "查看预留库存、待支付订单占用、可售库存和低库存风险。",
    group: "商品与库存",
    routePath: "/admin/reports/inventory/occupancy",
    legacyPaths: [],
    fetcher: fetchInventoryOccupancyReport as ReportRegistryItem["fetcher"],
    endpoint: "/admin/reports/inventory/occupancy",
    exportType: "inventory_occupancy",
    permission: "report.view",
    capability: "inventoryEnabled",
    filterProfile: "none",
    filters: [],
    kpiProfile: "inventory",
    kpiPriorityKeys: ["锁定库存", "预留库存", "待支付占用", "待支付订单数", "缺货SKU", "低库存SKU", "SKU数"],
    warningKeys: [],
    columns: ["product_name", "variant_name", "sku_code", "stock", "reserved_stock", "pending_order_locked_stock", "pending_order_count", "locked_stock", "available_stock", "warning_stock", "stock_status"],
    shellType: "generic",
    exportable: true,
    dataScopeNote: "库存占用同时展示 V2 reserved_stock 和旧链路待支付订单占用，用于排查秒杀、下单未支付和库存锁定问题。",
    maxKpis: 7,
  },
  {
    key: "order_analysis",
  ...
  },
  {
    ...
    reportKey: "order_cancel_reason",
    title: "取消原因",
    description: "按取消原因汇总订单数量、影响用户和取消金额，定位超时未付或运营问题。",
    group: "订单与客户",
    routePath: "/admin/reports/orders/cancel-reasons",
    legacyPaths: [],
    fetcher: fetchOrderCancelReasonReport as ReportRegistryItem["fetcher"],
    endpoint: "/admin/reports/orders/cancel-reasons",
    exportType: "order_cancel_reason",
    permission: "report.view",
    filterProfile: "dateOrder",
    filters: ["dateRange", "orderStatus", "paymentStatus", "paymentMethod"],
    kpiProfile: "order",
    kpiPriorityKeys: ["取消订单数", "影响用户数", "取消订单金额", "已支付取消金额"],
    warningKeys: [],
    columns: ["cancel_reason", "cancel_count", "user_count", "cancelled_amount", "paid_cancelled_amount", "first_cancelled_at", "latest_cancelled_at"],
    shellType: "generic",
    exportable: true,
    dataScopeNote: "取消原因按订单 cancelled_at 优先统计；旧数据缺取消时间时回退 updated_at 或 created_at。",
    maxKpis: 4,
  },
  {
    key: "payment_failure",
    reportKey: "payment_failure",
    title: "支付失败",
    description: "按网关、渠道、失败原因和复核状态汇总支付失败事件。",
    group: "订单与客户",
    routePath: "/admin/reports/payments/failures",
    legacyPaths: [],
    fetcher: fetchPaymentFailureReport as ReportRegistryItem["fetcher"],
    endpoint: "/admin/reports/payments/failures",
    exportType: "payment_failure",
    permission: "report.view",
    filterProfile: "date",
    filters: ["dateRange"],
    kpiProfile: "order",
    kpiPriorityKeys: ["失败事件数", "支付单数", "影响订单数", "需复核事件数"],
    warningKeys: [],
    columns: ["provider", "channel_code", "failure_reason", "event_type", "verify_status", "processing_result", "risk_level", "review_status", "failed_event_count", "payment_order_count", "affected_order_count", "expected_amount", "actual_amount", "latest_event_at"],
    shellType: "generic",
    exportable: true,
    dataScopeNote: "支付失败来自 payment_events，包含签名失败、金额/币种不一致、网关失败和需人工复核事件。",
    maxKpis: 4,
  },
  {
  ...
  {
    key: "promotion_conversion",
    reportKey: "promotion_conversion",
    title: "活动转化",
    description: "跟踪活动曝光、加购、提交订单、支付订单、销售额和优惠成本。",
    group: "营销分析",
    routePath: "/admin/reports/promotions/conversion",
    legacyPaths: [],
    fetcher: fetchPromotionConversionReport as ReportRegistryItem["fetcher"],
    endpoint: "/admin/reports/promotions/conversion",
    exportType: "promotion_conversion",
    permission: "report.view",
    filterProfile: "dateActivity",
    filters: ["dateRange", "activityId"],
    kpiProfile: "activity",
    kpiPriorityKeys: ["活动数", "浏览量", "加购量", "提交订单数", "支付订单数", "销售额", "优惠成本", "活动支付转化率"],
    warningKeys: [],
    columns: ["promotion_title", "promotion_type", "promotion_status", "start_at", "end_at", "product_count", "view_count", "add_cart_count", "order_submit_count", "usage_count", "confirmed_count", "released_count", "paid_order_count", "sales_amount", "discount_amount", "conversion_rate", "cost_per_paid_order"],
    shellType: "generic",
    exportable: true,
    dataScopeNote: "活动转化优先使用 analytics_events 的 activity_id 曝光/加购数据，并用 promotion_usages 统计实际命中和支付。",
    maxKpis: 8,
  },
  {
    key: "coupon_analysis",
  ...
  {
    key: "discount_cost",
    reportKey: "discount_cost",
    title: "优惠成本",
    description: "按时间汇总活动、优惠券、积分、返现和运费减免成本。",
    group: "营销分析",
    routePath: "/admin/reports/discounts/cost",
    legacyPaths: [],
    fetcher: fetchDiscountCostReport as ReportRegistryItem["fetcher"],
    endpoint: "/admin/reports/discounts/cost",
    exportType: "discount_cost",
    permission: "report.view",
    filterProfile: "date",
    filters: ["dateRange", "granularity"],
    supportsGranularity: true,
    kpiProfile: "coupon",
    kpiPriorityKeys: ["优惠成本", "实收金额", "优惠成本率", "活动优惠", "优惠券优惠", "积分抵扣", "返现抵扣", "运费减免", "支付订单数"],
    warningKeys: [],
    columns: ["date", "order_count", "paid_order_count", "paid_amount", "activity_discount_amount", "coupon_discount_amount", "points_discount_amount", "reward_cash_discount_amount", "shipping_discount_amount", "total_discount_amount", "discount_rate"],
    shellType: "generic",
    exportable: true,
    dataScopeNote: "优惠成本仅统计已支付且未取消订单；旧库缺细分字段时对应列降级为 0。",
    maxKpis: 9,
  },
  {
    key: "search_analysis",
