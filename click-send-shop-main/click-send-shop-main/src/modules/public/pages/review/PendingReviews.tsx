...
export default function PendingReviews() {
  ...
  return (
    <div className="min-h-screen bg-background p-4">
      ...
      {!loading && items.length === 0 ? <p className="text-sm text-muted-foreground">暂无待评价商品，确认收货后可评价。</p> : null}
          ...
          <div key={orderId} className="rounded-xl border p-3">
            ...
            {list.map((it) => (
              <div key={it.order_item_id} className="mb-2 flex items-center gap-3">
                ...
                <button className="rounded-full border px-3 py-1 text-xs" onClick={() => setOrderItemId(it.order_item_id)}>写评价</button>
              </div>
            ))}
          </div>
        ))}
