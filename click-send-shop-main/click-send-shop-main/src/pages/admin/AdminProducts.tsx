    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button type="button" onClick={() => navigate("/admin/categories")} className="touch-manipulation flex min-h-[44px] items-center gap-1.5 rounded-xl border border-border px-4 py-2.5 text-sm text-foreground hover:bg-secondary active:bg-secondary/80"><FolderTree size={16} /> 分类</button>
          <button type="button" onClick={() => navigate("/admin/tags")} className="touch-manipulation flex min-h-[44px] items-center gap-1.5 rounded-xl border border-border px-4 py-2.5 text-sm text-foreground hover:bg-secondary active:bg-secondary/80"><Tags size={16} /> 标签</button>
          <PermissionGate permission="product.view">
            <button type="button" onClick={handleExportCsv} className="touch-manipulation flex min-h-[44px] items-center gap-1.5 rounded-xl border border-border px-4 py-2.5 text-sm text-foreground hover:bg-secondary active:bg-secondary/80"><Download size={16} /> 导出</button>
          </PermissionGate>
          <PermissionGate permission="product.manage">
            <label className="touch-manipulation flex min-h-[44px] cursor-pointer items-center gap-1.5 rounded-xl border border-border px-4 py-2.5 text-sm text-foreground hover:bg-secondary active:bg-secondary/80">
              <Upload size={16} /> 导入
              <input type="file" accept=".csv,text/csv" className="hidden" onChange={handleImportCsv} />
            </label>
          </PermissionGate>
          <PermissionGate permission="product.manage">
            <button type="button" onClick={() => navigate("/admin/products/new")} className="touch-manipulation flex min-h-[44px] items-center gap-1.5 rounded-xl bg-gold px-4 py-2.5 text-sm font-semibold text-primary-foreground active:opacity-90"><Plus size={16} /> 新增</button>
          </PermissionGate>
        </div>
      </div>
      ...
      {selected.length > 0 && (
        <PermissionGate permission="product.manage">
          <div className="flex flex-wrap items-center gap-2 rounded-xl border border-gold/30 bg-gold/5 px-3 py-3 sm:px-4">
            ...
            <button type="button" onClick={() => batchToggleStatus("active")} className="touch-manipulation flex min-h-[40px] items-center gap-1 rounded-lg border border-border px-3 py-2 text-xs text-foreground hover:bg-secondary"><Eye size={14} /> 批量上架</button>
            <button type="button" onClick={() => batchToggleStatus("inactive")} className="touch-manipulation flex min-h-[40px] items-center gap-1 rounded-lg border border-border px-3 py-2 text-xs text-foreground hover:bg-secondary"><EyeOff size={14} /> 批量下架</button>
          </div>
        </PermissionGate>
      )}
          ...
          <div key={p.id} className="rounded-2xl border border-border bg-card p-4 shadow-sm">
            <div className="flex gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 text-sm text-muted-foreground">RM {p.price} · 库存 {p.stock === 0 ? <span className="text-destructive">缺货</span> : p.stock}</p>
                ...
                <div className="mt-3 flex gap-2">
                  <PermissionGate permission="product.manage">
                    <button type="button" onClick={() => navigate(`/admin/products/${p.id}`)} className="touch-manipulation flex min-h-[44px] flex-1 items-center justify-center gap-1 rounded-xl border border-border py-2 text-sm font-medium text-foreground active:bg-secondary">
                      <Pencil size={16} /> 编辑
                    </button>
                  </PermissionGate>
                  <PermissionGate permission="product.manage">
                    <button type="button" onClick={() => toggleSingleStatus(p.id)} className="touch-manipulation flex min-h-[44px] flex-1 items-center justify-center gap-1 rounded-xl border border-border py-2 text-sm active:bg-secondary">
              ...
              <tr key={p.id} className="border-b border-border last:border-0 hover:bg-secondary/30">
                ...
                <td className="px-4 py-3">
                  <div className="flex gap-1">
                    ...
                    {p.is_recommended && <span className="rounded bg-gold/10 px-1.5 py-0.5 text-[10px] text-gold">推荐</span>}
                  </div>
                </td>
                ...
                <td className="px-4 py-3">
                  <div className="flex gap-1">
                    <PermissionGate permission="product.manage">
                      <button type="button" onClick={() => navigate(`/admin/products/${p.id}`)} className="touch-manipulation rounded-lg border border-border p-2 text-muted-foreground hover:bg-secondary"><Pencil size={14} /></button>
                    </PermissionGate>
                    <PermissionGate permission="product.manage">
                      <button type="button" onClick={() => toggleSingleStatus(p.id)} className="touch-manipulation rounded-lg border border-border p-2 text-muted-foreground hover:bg-secondary">
