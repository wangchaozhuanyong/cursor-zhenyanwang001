  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <select value={dateRange} onChange={(e) => setDateRange(e.target.value)} className="rounded-xl border border-border bg-card px-3 py-2 text-xs outline-none">
      ...
      <div className="rounded-2xl border border-border bg-card p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <PermissionGate permission="report.export">
            <button type="button" onClick={() => handleExport("sales")} className="flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium text-gold hover:bg-gold/10">
        ...
        {salesChart.length > 0 ? (
          <ResponsiveContainer width="100%" height={240}>
            <AreaChart data={salesChart}>
              ...
              <Tooltip />
              <Area type="monotone" dataKey="revenue" stroke="hsl(43,72%,52%)" fill="url(#revGrad)" strokeWidth={2} name="销售额" />
            </AreaChart>
          </ResponsiveContainer>
        ) : (
      ...
      <div className="rounded-2xl border border-border bg-card p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <PermissionGate permission="report.export">
            <button type="button" onClick={() => handleExport("users")} className="flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium text-gold hover:bg-gold/10">
              <Download size={12} /> 导出
            </button>
          </PermissionGate>
        </div>
        {userChart.length > 0 ? (
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={userChart}>
              ...
              <YAxis tick={{ fontSize: 11 }} />
              <Tooltip />
              <Bar dataKey="newUsers" fill="hsl(43,72%,52%)" radius={[4, 4, 0, 0]} name="新用户" />
            </BarChart>
          </ResponsiveContainer>
        ) : (
      ...
      <div className="rounded-2xl border border-border bg-card p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <PermissionGate permission="report.export">
            <button type="button" onClick={() => handleExport("products")} className="flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium text-gold hover:bg-gold/10">
              <Download size={12} /> 导出
            </button>
          </PermissionGate>
        </div>
          ...
          {topProducts.slice(0, 10).map((item, i) => {
            ...
            return (
              <div key={String(item.id ?? i)} className="flex flex-wrap items-center gap-2 sm:gap-3">
                <span className={`flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full text-xs font-bold ${i < 3 ? "bg-gold text-primary-foreground" : "bg-secondary text-muted-foreground"}`}>{i + 1}</span>
                ...
                <span className="text-xs font-medium text-gold flex-shrink-0">RM {Number(item.totalRevenue || 0).toFixed(2)}</span>
              </div>
            );
          })}
