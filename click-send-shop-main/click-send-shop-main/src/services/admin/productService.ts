...
export async function batchDeleteProducts(ids: string[]): Promise<ProductBatchDeleteResult> {
  const res = await productApi.batchDeleteProducts(ids);
  return res.data;
}
...
export async function exportProductsCsv(params?: ProductListParams) {
  ...
  if (params?.media_status) qs.set("media_status", params.media_status);
