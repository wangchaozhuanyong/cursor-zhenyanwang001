...
/** 报表列名兜底：常见英文片段 → 中文（未在 REPORT_COLUMN_LABELS 登记时拼接） */
const REPORT_COLUMN_TOKEN_LABELS: Record<string, string> = {
  units: "件数",
  per: "每",
  order: "订单",
  paid: "已支付",
...
function humanizeColumnKey(key: string): string {
  if (key.endsWith("_id")) {
    ...
    return REPORT_COLUMN_LABELS[base] ?? `${REPORT_COLUMN_TOKEN_LABELS[base] || base}编号`;
  }
  const parts = key.split("_").filter(Boolean);
  const labeled = parts.map((part) => REPORT_COLUMN_TOKEN_LABELS[part] ?? part);
  const hasUnknownLatin = labeled.some((p) => /^[a-z]+$/i.test(p));
  if (hasUnknownLatin) return `字段（${key}）`;
  return labeled.join("");
}
