...
export const DATA_CLEANUP_POLICY_LABELS: Record<string, string> = {
  ...
  data_cleanup_runs_preview: "清理预览记录",
  data_cleanup_runs_history: "清理执行历史",
  data_cleanup_run_steps_history: "清理步骤历史",
  data_cleanup_file_candidates_history: "文件清理候选历史",
  backup_alerts_resolved: "已解决备份告警",
  backup_jobs_history: "备份任务历史",
  restore_jobs_history: "恢复任务历史",
  restore_drill_reports_history: "恢复演练报告历史",
  binlog_files_history: "Binlog 文件元数据历史",
  search_terms_inactive: "低频搜索词",
  coupon_events_history: "优惠券事件流水",
  cache_meta_expired: "失效缓存元数据",
  admin_event_actions_history: "后台事件操作历史",
  admin_event_user_states_history: "后台事件用户状态历史",
  raw_upload_objects: "S3 原始临时上传对象",
  orphan_upload_files_preview: "孤儿上传文件预览",
  orphan_upload_files_run: "孤儿上传文件执行",
};
...
export const DATA_CLEANUP_TABLE_LABELS: Record<string, string> = {
  ...
  data_repair_tasks: "数据修复任务",
  data_cleanup_runs: "数据清理运行记录",
  data_cleanup_run_steps: "数据清理步骤记录",
  data_cleanup_file_candidates: "文件清理候选记录",
  backup_alerts: "备份告警",
  backup_jobs: "备份任务",
  restore_jobs: "恢复任务",
  restore_drill_reports: "恢复演练报告",
  binlog_files: "Binlog 文件元数据",
  search_terms: "搜索词",
  coupon_events: "优惠券事件流水",
  cache_meta: "缓存元数据",
  admin_event_actions: "后台事件操作历史",
  admin_event_user_states: "后台事件用户状态",
  "s3:uploads/raw": "S3 原始临时上传目录",
  uploads: "上传文件目录",
};
