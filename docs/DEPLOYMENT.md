...
| PM2 | `server/ecosystem.config.cjs` |
...
5. **启动**：在 `server` 目录 `npm run start:prod`，或使用 `pm2 start ecosystem.config.cjs --env production`。
6. **HTTPS**：使用 Caddy / Nginx / 云负载均衡终止 TLS，反代到 `127.0.0.1:3000`（参考 `deploy/` 下示例）。
7. **备份**：计划任务定期执行 `scripts/backup-mysql.ps1`，并将 `backups/` 拷到异地。
