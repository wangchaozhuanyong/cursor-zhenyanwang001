...
确认弹窗、底部抽屉、表单提交区和下载/删除/开启/保存等高意图操作中，按钮内容必须按“图标在左、文字在右”排列，图标与文字保持稳定间距。禁止在这类场景继续使用只有纯文字的圆角按钮；确需使用纯图标按钮时必须提供明确的 `aria-label`，并保证周围上下文足够清楚。

## 10.1 新品入口规则

前台「新品上市」不是独立专题页。首页新品区块的「查看更多」、商品卡新品标签、首页导航兜底新品入口、旧 `/new-arrivals` 路由，都必须统一收口到 `/categories?is_new=1&home_new_arrivals_rule=1`。分类页顶部固定保留「全部」「新品」两个系统入口，普通分类从第三个入口开始展示，后台分类管理不得把这两个入口当作可删除的普通分类。
