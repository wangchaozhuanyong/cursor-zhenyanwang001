...
const { VIBRANT_SUNSET_CORAL_SKIN_ID } = require('../src/modules/user/starterThemeSkins');
...
module.exports = {
  async down(query) {
    ...
    const filtered = payload.skins.filter((s) => s.id !== VIBRANT_SUNSET_CORAL_SKIN_ID);
    if (filtered.length === payload.skins.length) return;

    const next = normalizeThemeSkinsPayload({
      defaultSkinId: payload.defaultSkinId,
      activeSkinId:
        payload.activeSkinId === VIBRANT_SUNSET_CORAL_SKIN_ID
