...
const { HAUTE_BLANC_SKIN, HAUTE_BLANC_SKIN_ID } = require('../src/modules/user/starterThemeSkins');

/**
 * 将推荐皮肤「大马通·高定流白」合并进 theme_skins（已存在则跳过）。
...
module.exports = {
  async up(query) {
    ...
    if (payload.skins.some((s) => s.id === HAUTE_BLANC_SKIN_ID)) {
      return;
    }
    ...
    const next = normalizeThemeSkinsPayload({
      ...
      skins: [
        ...
        {
          ...HAUTE_BLANC_SKIN,
          config: normalizeThemeConfig(HAUTE_BLANC_SKIN.config),
        },
      ],
    });
