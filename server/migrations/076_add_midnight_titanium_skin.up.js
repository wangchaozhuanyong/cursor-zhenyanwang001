...
const { MIDNIGHT_TITANIUM_SKIN, MIDNIGHT_TITANIUM_SKIN_ID } = require('../src/modules/user/starterThemeSkins');

/**
 * 将推荐皮肤「幻夜钛银·大马通」合并进 theme_skins（已存在则跳过）。
...
module.exports = {
  async up(query) {
    ...
    if (payload.skins.some((s) => s.id === MIDNIGHT_TITANIUM_SKIN_ID)) {
      return;
    }
    ...
    const next = normalizeThemeSkinsPayload({
      ...
      skins: [
        ...payload.skins,
        {
          ...
          config: normalizeThemeConfig(MIDNIGHT_TITANIUM_SKIN.config),
        },
      ],
    });
