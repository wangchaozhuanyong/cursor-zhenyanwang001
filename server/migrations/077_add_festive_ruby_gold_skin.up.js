...
const { FESTIVE_RUBY_GOLD_SKIN, FESTIVE_RUBY_GOLD_SKIN_ID } = require('../src/modules/user/starterThemeSkins');

/**
 * 将推荐皮肤「大马通限定·瑞红鎏金」合并进 theme_skins（已存在则跳过）。
...
module.exports = {
  async up(query) {
    ...
    if (payload.skins.some((s) => s.id === FESTIVE_RUBY_GOLD_SKIN_ID)) {
      return;
    }
    ...
    const next = normalizeThemeSkinsPayload({
      ...
      skins: [
        ...payload.skins,
        {
          ...
          config: normalizeThemeConfig(FESTIVE_RUBY_GOLD_SKIN.config),
        },
      ],
    });
