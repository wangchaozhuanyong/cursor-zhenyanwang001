...
const { AETHERIAL_BLANC_SKIN, AETHERIAL_BLANC_SKIN_ID } = require('../src/modules/user/starterThemeSkins');

/**
 * 将推荐皮肤「无界谧白·空间建构」合并进 theme_skins（已存在则跳过）。
...
module.exports = {
  async up(query) {
    ...
    if (payload.skins.some((s) => s.id === AETHERIAL_BLANC_SKIN_ID)) {
      return;
    }
    ...
    const next = normalizeThemeSkinsPayload({
      ...
      skins: [
        ...payload.skins,
        {
          ...
          config: normalizeThemeConfig(AETHERIAL_BLANC_SKIN.config),
        },
      ],
    });
