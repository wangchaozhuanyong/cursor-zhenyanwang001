...
const { ORGANIC_SANDSTONE_SKIN, ORGANIC_SANDSTONE_SKIN_ID } = require('../src/modules/user/starterThemeSkins');

/**
 * 将推荐皮肤「质朴之境·原生砂岩」合并进 theme_skins（已存在则跳过）。
...
module.exports = {
  async up(query) {
    ...
    if (payload.skins.some((s) => s.id === ORGANIC_SANDSTONE_SKIN_ID)) {
      return;
    }
    ...
    const next = normalizeThemeSkinsPayload({
      ...
      skins: [
        ...payload.skins,
        {
          ...
          config: normalizeThemeConfig(ORGANIC_SANDSTONE_SKIN.config),
        },
      ],
    });
