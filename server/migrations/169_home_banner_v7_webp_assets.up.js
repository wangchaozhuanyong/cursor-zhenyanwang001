...
module.exports = {
  async up(query) {
    const newIds = BANNERS.map((banner) => banner.id);
    await query(
      `UPDATE banners
          SET enabled = 0,
        ...
        WHERE deleted_at IS NULL
          AND id NOT IN (${newIds.map(() => '?').join(',')})
          AND (
            image IN (${LEGACY_IMAGES.map(() => '?').join(',')})
            OR title IN (${LEGACY_TITLES.map(() => '?').join(',')})
          )`,
      [...newIds, ...LEGACY_IMAGES, ...LEGACY_TITLES],
    );

    for (const banner of BANNERS) {
      await query(
        `INSERT INTO banners
          (id, title, description, cta_text, image, link, sort_order, enabled, publish_status, deleted_at, last_modified_at)
