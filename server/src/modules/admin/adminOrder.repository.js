 ...
 *
 * 仅做数据库访问。函数命名以「动�?+ 表」为主：
 *   selectXxx / updateXxx / insertXxx / countXxx
 *
 * 凡涉及多语句事务的方法，第一参数都接�?`q`（pool �?connection）以便服务层
 * 在同一个事务内串联多次写入。Service 不应再出�?db.query / conn.query�? */
...
/**
 * 进入「已发货」时记录首次发货时间（若已有则保留，便于多次更新运单号不改变计时起点�? * @param {import('mysql2/promise').PoolConnection} q
 * @param {string} orderId
 */
