...
async function selectAllBanners() {
  try {
    ...
    if (e && (e.code === 'ER_BAD_FIELD_ERROR' || /Unknown column.*deleted_at/i.test(msg))) {
...
async function insertBanner(params) {
  ...
  try {
    await db.query(
      'INSERT INTO banners (id, title, description, cta_text, image, image_mobile, image_desktop, link, sort_order, enabled, publish_status, last_modified_by, last_modified_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,NOW())',
      [id, title || '', description || '', cta_text || '', image, image_mobile || '', image_desktop || '', link || '', sort_order || 0, enabled, publish_status || 'published', last_modified_by || null],
    );
  } catch (e) {
    const msg = String(e && e.sqlMessage ? e.sqlMessage : e.message || e);
    if (e && e.code === 'ER_BAD_FIELD_ERROR') {
      // Backward-compatible insert for instances missing newer audit columns.
      if (/Unknown column.*image_(mobile|desktop)/i.test(msg)) {
        await db.query(
          'INSERT INTO banners (id, title, description, cta_text, image, link, sort_order, enabled, publish_status, last_modified_by, last_modified_at) VALUES (?,?,?,?,?,?,?,?,?,?,NOW())',
          [id, title || '', description || '', cta_text || '', image, link || '', sort_order || 0, enabled, publish_status || 'published', last_modified_by || null],
        );
        return;
      }
      if (/Unknown column.*cta_text/i.test(msg)) {
        await db.query(
      ...
      if (hasPublishStatus) {
        await db.query(
          'INSERT INTO banners (id, title, image, link, sort_order, enabled, publish_status) VALUES (?,?,?,?,?,?,?)',
          [id, title || '', image, link || '', sort_order || 0, enabled, publish_status || 'published'],
        );
      } else {
        await db.query(
          'INSERT INTO banners (id, title, image, link, sort_order, enabled) VALUES (?,?,?,?,?,?)',
...
async function deleteBanner(id, deletedBy) {
  try {
    ...
    const msg = String(e && e.sqlMessage ? e.sqlMessage : e.message || e);
    if (e && (e.code === 'ER_BAD_FIELD_ERROR' || /Unknown column.*deleted_(at|by)/i.test(msg))) {
...
async function restoreBanner(id) {
  try {
    ...
    const msg = String(e && e.sqlMessage ? e.sqlMessage : e.message || e);
    if (e && (e.code === 'ER_BAD_FIELD_ERROR' || /Unknown column.*deleted_(at|by)/i.test(msg))) {
      // Old schema without soft-delete columns has no recoverable recycle-bin state.
      return;
    }
    throw e;
  }
...
async function selectProductTags() {
  ...
  `;
  const sqlLegacy = `
    SELECT pt.id, pt.name, pt.sort_order, '閲戣壊' AS color,
...
async function insertProductTag(id, name, color, bgColor, textColor, sortOrder = 0, enabled = 1) {
  const c = color && String(color).trim() ? String(color).trim().slice(0, 20) : '閲戣壊';
...
/** 将指定模板设为唯一默认且唯一启用（其余全部停用） */
async function activateShippingTemplateAsDefault(id) {
  await db.query('UPDATE shipping_templates SET enabled = 0, is_default = 0');
  await db.query('UPDATE shipping_templates SET enabled = 1, is_default = 1 WHERE id = ?', [id]);
}

async function selectShippingTemplateByIdAny(id) {
  const [[row]] = await db.query('SELECT id, enabled, is_default FROM shipping_templates WHERE id = ?', [id]);
  return row || null;
}

async function ensureShippingTemplateHasDefault() {
  const [[active]] = await db.query(
    'SELECT id FROM shipping_templates WHERE enabled = 1 AND is_default = 1 ORDER BY id ASC LIMIT 1',
  );
  ...
  if (enabled?.id != null) {
    await db.query('UPDATE shipping_templates SET is_default = 0');
    await db.query('UPDATE shipping_templates SET is_default = 1 WHERE id = ?', [enabled.id]);
    return enabled.id;
  }

  const [[first]] = await db.query('SELECT id FROM shipping_templates ORDER BY id ASC LIMIT 1');
  if (first?.id != null) {
    await activateShippingTemplateAsDefault(first.id);
    return first.id;
  }
  return null;
}
...
/** 鍐呭椤垫洿鏂板墠瀹¤蹇収锛堜粎 id / title / slug / content 闀垮害锛?*/
...
module.exports = {
  ...
  activateShippingTemplateAsDefault,
  selectShippingTemplateByIdAny,
  ensureShippingTemplateHasDefault,
  selectPointsRules,
