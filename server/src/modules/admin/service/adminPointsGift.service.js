...
const giftRepo = require('../../loyalty/repository/pointsGift.repository');
...
async function listGiftItems(query = {}) {
  const page = await giftRepo.selectGiftItemsPage(orderRepo.getPool(), query);
  const list = [];
...
async function createGiftItem(body) {
  ...
  const product = await catalogService.getProductById(payload.product_id);
  if (!product) throw new BusinessError(400, '关联商品不存在');
  const id = generateId();
  await giftRepo.insertGiftItem(orderRepo.getPool(), { id, ...payload });
  return { data: { id, ...payload }, message: '礼品已创建' };
}

async function updateGiftItem(id, body) {
  const existing = await giftRepo.selectGiftItemById(orderRepo.getPool(), id);
  ...
  await giftRepo.updateGiftItem(orderRepo.getPool(), id, fields, values);
...
async function deleteGiftItem(id) {
  const existing = await giftRepo.selectGiftItemById(orderRepo.getPool(), id);
  if (!existing) throw new BusinessError(404, '礼品不存在');
  await giftRepo.deleteGiftItem(orderRepo.getPool(), id);
  return { data: null, message: '礼品已删除' };
}

async function listRedemptions(query = {}) {
  return giftRepo.selectGiftRedemptionsPage(orderRepo.getPool(), query);
}
