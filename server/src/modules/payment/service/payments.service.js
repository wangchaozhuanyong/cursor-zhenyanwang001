...
function requireOrderApi(name) {
  ...
  if (typeof fn !== 'function') {
    throw new Error(`Order 模块 API 未暴露方�? ${name}`);
  }
  return fn;
}
...
function requireUserApi(name) {
  ...
  if (typeof fn !== 'function') {
    throw new Error(`User 模块 API 未暴露方�? ${name}`);
  }
  return fn;
}
...
function requireMyinvoisApi(name) {
  ...
  if (typeof fn !== 'function') {
    throw new Error(`MyInvois 模块 API 未暴露方�? ${name}`);
  }
  return fn;
}
...
async function payWithRewardWallet(userId, orderId) {
  ...
  try {
    ...
    if (balance < payableAmount) {
      throw new ValidationError(`返现钱包余额不足，当前可�?RM ${balance.toFixed(2)}`);
    }
...
async function createStripeCheckoutForOrder(userId, orderId, returnUrlHint, idempotencyKey) {
  ...
  if (!secretKey) throw new BusinessError(503, 'Stripe 未配�?STRIPE_SECRET_KEY');
...
async function createIntent(userId, body) {
  ...
  if (!orderId || !channelCode) {
    throw new ValidationError('order_id �?channel_code 必填');
  }
  ...
  if (channel.provider === 'internal' && channel.code === 'reward_wallet') {
    throw new ValidationError('返现钱包请使�?POST /orders/:id/pay channel=reward_wallet');
  }
...
async function recordRefundByAdmin(req, orderId, body) {
  ...
  try {
    ...
    await writeAuditLog({
      ...
      objectType: 'order',
      objectId: order.id,
      summary: `记录退�?RM ${amount.toFixed(2)} (${isFullRefund ? '全额' : '部分'})`,
...
async function createReconciliation(req, body) {
  ...
  if (!reconcileDate || !provider) throw new ValidationError('reconcile_date �?provider 必填');
...
async function handleMalaysiaLocalWebhook(provider, body, headers = {}) {
  ...
  if (!expectedSecret) {
    throw new BusinessError(503, '未配�?PAYMENT_MALAYSIA_WEBHOOK_SECRET');
  }
  if (headerSecret) {
  ...
  try {
    ...
    if (!paymentOrderId) throw new ValidationError('payment_order_id 或可匹配�?order_id 必填');
