...
exports.bindWechat = asyncRoute(async (req, res) => {
  const url = await meService.startWechatBind(req.user.id, req.body?.redirect);
  res.success({ authorizeUrl: url }, '请在新窗口完成微信授权');
});

exports.unbindWechat = asyncRoute(async (req, res) => {
  const result = await meService.unbindWechatForUser(req.user.id);
  res.success(result.data, result.message);
});

exports.getWechatBinding = asyncRoute(async (req, res) => {
  const data = await meService.getWechatBindingForProfile(req.user.id);
  res.success(data);
});

exports.getSummary = asyncRoute(async (req, res) => {
  const data = await meService.getSummary(req.user.id);
  res.success(data);
});

exports.bindWechatStart = asyncRoute(async (req, res, next) => {
  try {
    const url = await meService.startWechatBind(req.user.id, req.query.redirect);
