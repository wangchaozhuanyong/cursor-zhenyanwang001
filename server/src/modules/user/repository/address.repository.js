...
async function setDefaultById(id, userId) {
...
async function updateAddressDynamic(conn, id, userId, fields, values) {
  if (fields.length === 0) return;
  values.push(id, userId);
  await conn.query(`UPDATE addresses SET ${fields.join(', ')} WHERE id = ? AND user_id = ?`, values);
}

async function selectRowByIdAndUser(id, userId) {
  const [[row]] = await db.query('SELECT * FROM addresses WHERE id = ? AND user_id = ?', [id, userId]);
  return row || null;
}
